    ...
    for (var j = 0; j < tabs.length; j++) {
        ...
        if (tabs[j] == 'Orders') {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                data[i].boxname ? data[i].boxname.name || (data[i].boxname = {
                    name: "",
                    sku: ""
                }) : data[i].boxname = {
                    name: "",
                    sku: ""
                }, data[i].packagingType ? data[i].packagingType.name || (data[i].packagingType = {
                    name: "",
                    sku: ""
            ...
            var flavkeys = flavmixArr.map(function(item) {
                return item.key;
            });
            var flavHeads = flavmixArr.map(function(item) {
                return item.head;
            });
            var keys = ['menu', 'notes', 'move', 'final_status', 'priority', 'batch', 'orderID', 'brand', 'productcode', 'productdescription', 'recipe.name', 'flavour.name', 'orderdate', 'QTY', 'VGval', 'AGval', 'PGval', 'Nico', 'Nicosalts', 'CBDvalue', 'MCTval', 'flavvalue', 'isFlavourMix', 'mixAmountNeeded','backedByPrimaryFlavour'];
            ...
            var heads = ['Menu', 'Notes', 'Move', 'Completed', 'Priority', 'Batch', 'Order ID', 'Brand', 'Code', 'Description', 'Recipe', 'Flavour', 'Order Date',
                'QTY(Litres)', 'VG (MG)', 'AG (MG)', 'PG (MG)', 'Nicotine (MG)', 'Nicotine Salts (MG)', 'CBD (MG)', 'MCT (MG)', 'Flavour (MG)', 'IS FLAVOUR MIX', 'Mix amount needed (MG)','Primary Flavour Mix Amount from Stock'
            ];
            ...
            var keys = ['Completed', 'priorities', 'prodDate', 'batches', 'customers', 'orderIDs', 'brands', 'MIXNAME', 'RECIPE', 'FLAVOUR', 'orderDates', 'QTYTOTAL', 'VGval', 'AGval', 'PGval', 'Nico', 'Nicosalts', 'CBDvalue', 'MCTval', 'flavvalue', 'isFlavourMix', 'mixAmountNeeded','backedByPrimaryFlavour'];
            ...
            var flavourMixData = base.getData('FlavourMixes');
            var recipeData = base.getData('Recipes');
            var fillData = base.getData('FillLevels');
            var packageData = base.getData('Packages');
            var boxData = base.getData('Boxes');
            ...
            var keyRow1 = ['prod', 'descr', 'unbrandSKU', 'unbranddescr', 'premixSKU', 'premixdescr', 'premixSKUColored', 'premixdescrColored', 'linkedBB', 'brand', 'brandSKU', 'btype', 'botSKU', 'fill', 'filllevels.time', 'lid', 'lidSKU',
                'packagingType.name', 'packagingType.sku', 'packages.botperPack', 'boxname.name', 'boxname.sku', 'boxes.divTubesForBox', 'NIB', 'flavour.name', 'flavour.sku'
            ]
            ...
            for (var i = 1; i <= 10; i++) {
                ...
                keyRow1 = keyRow1.concat(['fmix.[' + (i-1) + '].name', 'fmix.[' + (i-1) + '].sku', 'fmix.[' + (i-1) + '].val']);
            ...
            var keyRow = keyRow1.concat(['recipe.name', 'recipe.id',
                'recipes.VGrec', 'recipes.AGrec', 'recipes.PGrec', 'recipes.Nicorec', 'recipes.Nicorecsalts', 'recipes.cbdrec', 'recipes.MCTrecipe', 'recipes.Flavrec', 'recipes.vg', 'recipes.pg', 'recipes.strength', 'recipes.Color.name', 'recipes.Color.sku', 'recipes.Color.val',
                ...
                for (var k = 0; k < keyRow.length; k++) {
                    ...
                    if (keyRow) {
                        if (keyRow[k].indexOf('.') > -1) {
                            ...
                            switch (item[0]) {
                                case 'packages':
                                    var sku = data[i].packagingType ? data[i].packagingType.sku ? data[i].packagingType.sku : null : null;
                                    var dbItem = packageData[sku];
                                    item.shift();
                                    value = dbItem ? getProperty(dbItem, item.join('.')) : null;
                                    break;
                                case 'boxes':
                                    var sku = data[i].boxname ? data[i].boxname.sku ? data[i].boxname.sku : null : null;
                                    var dbItem = boxData[sku];
                                    item.shift();
                                    value = dbItem ? getProperty(dbItem, item.join('.')) : null;
                                    break;
                                case 'fmix':
                                    var sku = data[i].flavour ? data[i].flavour.sku ? data[i].flavour.sku : null : null;
                                    var dbItem = flavourMixData[sku];
                                if(sku == 'OOFLAV1998S'){
                                  Logger.log(1);
                                }
                                    if (dbItem) {
                                        item.shift();
                                        var flavourList = JSONtoARR(dbItem.flavours);
                                        value = getProperty(flavourList, item.join('.'));
                                     
                                    }
                                    break;
                                case 'filllevels':
                                    var sku = data[i].fill ? data[i].fill : null;
                                    var dbItem = fillData[sku];
                                    item.shift();
                                    value = dbItem ? getProperty(dbItem, item.join('.')) : null;
                                    break;
                                case 'recipes':
                                    var sku = data[i].recipe ? data[i].recipe.id ? data[i].recipe.id : null : null;
                                    var dbItem = recipeData[sku];
                                    item.shift();
                                    value = dbItem ? getProperty(dbItem, item.join('.')) : null;
                                    break;
                                default:
            ...
            var headerRow = ['SKU', 'Name', 'Supplier SKU', 'Supplier Name'];
            var keys = ['sku', 'name','supplierSKU','supplierName'];
            var values = [];
            for (var i = 0; i < data.length; i++) {
